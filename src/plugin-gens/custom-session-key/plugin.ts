  ...
  executeWithSessionKey: (
    args: Pick<
      EncodeFunctionDataParameters<
        typeof CustomSessionKeyPluginExecutionFunctionAbi,
        "executeWithSessionKey"
      >,
      "args"
    > &
...
export type InstallCustomSessionKeyPluginParams = {
...
type ManagementActions<
  ...
  TEntryPointVersion extends
    GetEntryPointFromAccount<TAccount> = GetEntryPointFromAccount<TAccount>,
> = {
  installCustomSessionKeyPlugin: (
    args: UserOperationOverridesParameter<TEntryPointVersion> &
      InstallCustomSessionKeyPluginParams &
      GetAccountParameter<TAccount> &
      GetContextParameter<TContext>,
  ) => Promise<SendUserOperationResult<TEntryPointVersion>>;
};
...
export type CustomSessionKeyPluginActions<
  TAccount extends SmartContractAccount | undefined =
    | SmartContractAccount
    | undefined,
  TContext extends UserOperationContext | undefined =
...
const addresses = {
  11155111: "0x33b1e1777d434E852B2084F62769f34fcc8A9189" as Address,
} as Record<number, Address>;

export const CustomSessionKeyPlugin: Plugin<typeof CustomSessionKeyPluginAbi> =
  {
    meta: {
    ...
    getContract: <C extends Client>(
      ...
      typeof CustomSessionKeyPluginAbi,
      PublicClient,
      Address
    > => {
...
export const customSessionKeyPluginActions: <
...
>(
...
) => CustomSessionKeyPluginActions<TAccount, TContext> = (client) => ({
  executeWithSessionKey({
    args,
    overrides,
    context,
    ...
    const uo = encodeFunctionData({
      abi: CustomSessionKeyPluginExecutionFunctionAbi,
  ...
  installCustomSessionKeyPlugin({
    ...
    const pluginAddress =
      ...
      (CustomSessionKeyPlugin.meta.addresses[chain.id] as Address | undefined);

    if (!pluginAddress) {
      throw new Error(
        "missing CustomSessionKeyPlugin address for chain " + chain.name,
      );
    }
...
export const CustomSessionKeyPluginExecutionFunctionAbi = [
  {
    type: "function",
...
export const CustomSessionKeyPluginAbi = [
  {
    type: "function",
    name: "addSessionKey",
    inputs: [
