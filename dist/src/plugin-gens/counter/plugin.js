...
const addresses = {
    11155111: "0x3e71215c32095fd32c458E683D557709c3cef2f9",
};
...
export const counterPluginActions = (client) => ({
    ...
    installCounterPlugin({ account = client.account, overrides, context, ...params }) {
        if (!account) {
            throw new AccountNotFoundError();
        }
        if (!isSmartAccountClient(client)) {
            throw new IncompatibleClientError("SmartAccountClient", "installCounterPlugin", client);
        }
        ...
        const pluginAddress = params.pluginAddress ??
            CounterPlugin.meta.addresses[chain.id];
        if (!pluginAddress) {
