...
async function installSavingsPlugin(extendedAccount) {
...
async function uninstallPlugin(address, extendAccount) {
...
async function createAutomation(extendedAccount) {
    const installedPlugins = await extendedAccount.getInstalledPlugins({});
    if (!installedPlugins.includes(SAVINGS_PLUGIN_ADDRESS)) {
        console.error("Savings plugin not installed. Run 'yarn start install' to install it.");
        return;
    }
